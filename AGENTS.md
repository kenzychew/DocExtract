...
core is invoked by three thin entry points (a folder watcher, a Gradio web
demo, and a FastAPI demo service in `demo/`) and depends on a swappable model
backend (Gemini free tier or local Ollama). Full detail in `docs/`.
...
Two demo entry points, both thin wrappers over `core.process_document`
(rule 1) — neither owns pipeline logic:

- `src/docfield/web/app.py` + root `app.py`: Gradio, deployed to the
  Hugging Face Space linked in the README.
- `demo/app.py`: FastAPI + a minimal static frontend (`demo/static/`),
  built for the uniform Railway-subdomain pattern used by the portfolio
  hub's other showcase pieces. Deps are pinned separately in
  `demo/requirements-demo.txt` (superset of the root `requirements.txt`,
  not constrained by Gradio's `mcp` extra). Run locally with
  `uv run uvicorn demo.app:app --reload`. Railway service config lives in
  `demo/railway.toml`, assuming the service's Root Directory is set to
  `demo/`; actual Railway service creation/wiring is a separate follow-up.
